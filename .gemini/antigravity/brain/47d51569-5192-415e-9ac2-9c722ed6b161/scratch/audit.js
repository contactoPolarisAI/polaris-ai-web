      ...
      try {
        ...
        const pageFindings = await page.evaluate((w) => {
          ...
          for (const el of allElements) {
            // Skip containers with too many children to avoid redundant matches, focus on leaf nodes
            ...
            if (style.position === 'fixed' || style.position === 'sticky') continue; // Handled separately

            // Check if right edge exceeds viewport or left edge is negative
            if (rect.right > innerWidth + 1 || rect.left < -1) {
              // Get selector or class info
          ...
          for (const el of targets) {
            ...
            if (rect.width < 44 || rect.height < 44) {
              ...
              // Flag if it's not inline (inline links in text paragraphs are exempt, but block buttons/links are not)
          ...
          for (const target of targetElements) {
            ...
            for (const fixed of fixedEls) {
              ...
              if (overlapX && overlapY) {
                // Confirm target contains visible content
